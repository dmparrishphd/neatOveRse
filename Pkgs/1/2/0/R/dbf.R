DBF.PROTO <- list(
	...
	info=function(.)
			list(
				DESCRIPTORS=list(
					RAW.PARSED=Dbf$.FIELD.DESCRIPTORS.RAW.PARSED,
					DESCRIPTORS=Dbf$FIELD.DESCRIPTORS,
					RAW=Dbf$.FIELD.DESCRIPTORS.RAW),

				HEAD=list(
					HEAD.PARSED=Dbf$.HEAD.PARSED,
					HEAD=Dbf$.HEAD),

				FIELDS=data.frame(
					stringsAsFactors=F,
					NAME=Dbf$FIELD.NAMES,
					TYPE=Dbf$FIELD.TYPES,
					LENGTH=Dbf$FIELD.LENGTHS,
					`colnames<-`(t(Dbf$FIELD.LAYOUT), c("FIRST", "LAST"))),

				SIZE=c(
					NRECORDS=Dbf$NRECORDS,
					RECSIZE=Dbf$RECSIZE,
					HEADSIZE=Dbf$HEADSIZE,
					NFIELDS=Dbf$NFIELDS)),
